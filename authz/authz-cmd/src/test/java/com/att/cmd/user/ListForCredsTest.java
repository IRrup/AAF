...
public class ListForCredsTest {
	
	private static ListForCreds lsForCreds;
	private static ListForCreds _lsForCreds;
	...
	public static void setUp () throws NoSuchFieldException, SecurityException, Exception, IllegalAccessException {
		...
		_lsForCreds = mock(ListForCreds.class);
		rreq = mock(Future.class);
		when(rreq.code()).thenReturn(HttpStatus.PARTIAL_CONTENT_206);
		when(_lsForCreds._exec(Mockito.anyInt(), Mockito.anyString(),Mockito.anyString())).thenCallRealMethod();
		...
		when(_lsForCreds.same(Mockito.any(Retryable.class))).thenReturn(206);
