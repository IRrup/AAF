...
public class RenameTest {
	...
	public void setUp () throws NoSuchFieldException, SecurityException, Exception, IllegalAccessException {
		...
		_rename = mock(Rename.class);
		rreq = mock(Future.class);
		when(rreq.code()).thenReturn(HttpStatus.PARTIAL_CONTENT_206);
		when(_rename._exec(Mockito.anyInt(), Mockito.anyString(),Mockito.anyString())).thenCallRealMethod();
		...
		when(_rename.same(Mockito.any(Retryable.class))).thenReturn(206);
	}
	
	@Test
	public void exec() {
		try {
			assertEquals(rename._exec(0, "add","del","reset","extend"),500);
	...
	public void exec_206() {
		try {
			assertEquals(_rename._exec(0, "add","del"),206);
		} catch (CadiException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		} catch (APIException e) {
