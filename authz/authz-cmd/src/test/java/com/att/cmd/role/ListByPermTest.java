...
public class ListByPermTest {
	...
	private static ListByPerm _lsByPerm;
	...
	public static void setUp () throws NoSuchFieldException, SecurityException, Exception, IllegalAccessException {
		...
		_lsByPerm = mock(ListByPerm.class);
		rreq = mock(Future.class);
		when(rreq.code()).thenReturn(HttpStatus.PARTIAL_CONTENT_206);
		when(_lsByPerm._exec(Mockito.anyInt(), Mockito.anyString(),Mockito.anyString())).thenCallRealMethod();
		...
		when(_lsByPerm.same(Mockito.any(Retryable.class))).thenReturn(206);
	}
	...
	public void exec_206() {
		try {
			assertEquals(_lsByPerm._exec(0, "add","del"),206);
		} catch (CadiException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		} catch (APIException e) {
