...
public class DescribeTest {
	...
	private static Describe describe;
	private static Describe _describe;
	...
	public void setUp () throws NoSuchFieldException, SecurityException, Exception, IllegalAccessException {
		...
		describe = new Describe(perm);
		_describe = mock(Describe.class);
		rreq = mock(Future.class);
		when(rreq.code()).thenReturn(HttpStatus.PARTIAL_CONTENT_206);
		when(_describe._exec(Mockito.anyInt(), Mockito.anyString(),Mockito.anyString())).thenCallRealMethod();
		...
		when(_describe.same(Mockito.any(Retryable.class))).thenReturn(206);
	}
	
	@Test
	public void exec() {
		try {
			assertEquals(describe._exec(0, "add","del","reset","extend"),500);
	...
	public void exec_206() {
		try {
			assertEquals(_describe._exec(0, "add","del"),206);
		} catch (CadiException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		} catch (APIException e) {
