...
public class ListForRolesTest {
	
	private static ListForRoles lsForRoles;
	private static ListForRoles _lsForRoles;
	...
	public static void setUp () throws NoSuchFieldException, SecurityException, Exception, IllegalAccessException {
		...
		_lsForRoles = mock(ListForRoles.class);
		rreq = mock(Future.class);
		when(rreq.code()).thenReturn(HttpStatus.PARTIAL_CONTENT_206);
		when(_lsForRoles._exec(Mockito.anyInt(), Mockito.anyString(),Mockito.anyString())).thenCallRealMethod();
		...
		when(_lsForRoles.same(Mockito.any(Retryable.class))).thenReturn(206);
