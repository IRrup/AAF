...
public class CreateTest {
	...
	public void setUp () throws NoSuchFieldException, SecurityException, Exception, IllegalAccessException {
		...
		_create = mock(Create.class);
		rreq = mock(Future.class);
		when(rreq.code()).thenReturn(HttpStatus.PARTIAL_CONTENT_206);
		when(_create._exec(Mockito.anyInt(), Mockito.anyString(),Mockito.anyString())).thenCallRealMethod();
		...
		when(_create.same(Mockito.any(Retryable.class))).thenReturn(206);
	}
	...
	public void exec_206() {
		try {
			assertEquals(_create._exec(0, "add","del"),206);
		} catch (CadiException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		} catch (APIException e) {
