...
public class ListByNSTest {
	
	private static ListByNS lsByNS;
	private static ListByNS _lsByNS;
	...
	public static void setUp () throws NoSuchFieldException, SecurityException, Exception, IllegalAccessException {
		...
		_lsByNS = mock(ListByNS.class);
		rreq = mock(Future.class);
		when(rreq.code()).thenReturn(HttpStatus.PARTIAL_CONTENT_206);
		when(_lsByNS._exec(Mockito.anyInt(), Mockito.anyString(),Mockito.anyString())).thenCallRealMethod();
		...
		when(_lsByNS.same(Mockito.any(Retryable.class))).thenReturn(206);
	}
	...
	public void exec_206() {
		try {
			assertEquals(_lsByNS._exec(0, "add","del"),206);
		} catch (CadiException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		} catch (APIException e) {
