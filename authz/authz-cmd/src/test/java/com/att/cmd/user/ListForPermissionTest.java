...
public class ListForPermissionTest {
	
	private static ListForPermission lsForPermission;
	private static ListForPermission _lsForPermission;
	...
	public static void setUp () throws NoSuchFieldException, SecurityException, Exception, IllegalAccessException {
		...
		_lsForPermission = mock(ListForPermission.class);
		rreq = mock(Future.class);
		when(rreq.code()).thenReturn(HttpStatus.PARTIAL_CONTENT_206);
		when(_lsForPermission._exec(Mockito.anyInt(), Mockito.anyString(),Mockito.anyString())).thenCallRealMethod();
		...
		when(_lsForPermission.same(Mockito.any(Retryable.class))).thenReturn(206);
