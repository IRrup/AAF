...
public class ListDelegatesTest {
	
	private static ListDelegates lsDelegates;
	private static ListDelegates _lsDelegates;
	...
	public static void setUp () throws NoSuchFieldException, SecurityException, Exception, IllegalAccessException {
		...
		_lsDelegates = mock(ListDelegates.class);
		rreq = mock(Future.class);
		when(rreq.code()).thenReturn(HttpStatus.PARTIAL_CONTENT_206);
		when(_lsDelegates._exec(Mockito.anyInt(), Mockito.anyString(),Mockito.anyString())).thenCallRealMethod();
		...
		when(_lsDelegates.same(Mockito.any(Retryable.class))).thenReturn(206);
