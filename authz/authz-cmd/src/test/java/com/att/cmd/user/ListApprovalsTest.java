...
public class ListApprovalsTest {
	...
	private static ListApprovals _lsApprovals;
	...
	public static void setUp () throws NoSuchFieldException, SecurityException, Exception, IllegalAccessException {
		...
		_lsApprovals = mock(ListApprovals.class);
		rreq = mock(Future.class);
		when(rreq.code()).thenReturn(HttpStatus.PARTIAL_CONTENT_206);
		when(_lsApprovals._exec(Mockito.anyInt(), Mockito.anyString(),Mockito.anyString())).thenCallRealMethod();
		...
		when(_lsApprovals.same(Mockito.any(Retryable.class))).thenReturn(206);
